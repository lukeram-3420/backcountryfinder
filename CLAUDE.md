...
for that course and check type. (The legacy `activity_mappings` branch
was removed alongside the activity-mismatch check in Initiative 1; the
`validator_price_exceptions` branch was removed in Initiative 4 when the
outlier check itself was deleted — the table persists in Supabase but is
orphaned and drops at V2 Phase 7.)
...
| — | Duplicate detection simplification (Initiative 6 of data quality mission) | Planned — not started. Strip the duplicate check to a pure scraper-signal: keep auto-hide, delete `validator_whitelist` load + lookup, delete `admin-diagnose-duplicate` edge function, replace Diagnose/Whitelist buttons in the Flags tab with a read-only row that links to the offending scraper file. Priority stack collapses to 2 layers (suppressions → summary exceptions) |
| — | Course count drop → Providers tab (Initiative 7 of data quality mission) | Planned — not started. Move the >30% count-drop signal off the Flags tab and onto the Providers tab. Delete `check_course_count()` from `validate_provider.py` entirely; Providers tab reads `scraper_run_log` on load and renders a yellow warning badge linking to GitHub Actions. `validator_warnings` types narrow to `null_avail` + `all_sold` (supersedes Initiative 4's wording) |
...
- **Initiative 6 — Duplicate detection simplification:** planned, not started. Strips the admin decision path from duplicate handling: `validator_whitelist` load + lookup deleted, `admin-diagnose-duplicate` edge function deleted, Diagnose/Whitelist buttons removed from the Flags tab. Auto-hide stays. Duplicate rows become read-only scraper-signal with a link to the offending `scraper_{provider_id}.py` on GitHub. Priority stack collapses to 2 layers (suppressions → summary exceptions).
- **Initiative 7 — Course count drop → Providers tab:** planned, not started. Deletes `check_course_count()` from `validate_provider.py`; Providers tab reads `scraper_run_log` on load and renders a yellow warning badge linking to GitHub Actions when the last-two-runs drop exceeds 30%. `validator_warnings` types narrow to `null_avail` + `all_sold`. No new tables, no Supabase SQL.
