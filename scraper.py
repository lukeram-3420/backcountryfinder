...
def find_place_id(provider_name, location):
    if not GOOGLE_PLACES_API_KEY:
        return None
    try:
        # Clean location — strip anything after / or , to get just the city name
        # e.g. "Penticton / Skaha Bluffs" -> "Penticton"
        # e.g. "Vancouver Island, BC" -> "Vancouver Island"
        import re as _re
        clean_location = _re.split(r"[/,]", location)[0].strip() if location else ""
        r = requests.get(
