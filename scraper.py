...
# Both carry the date in href — we just check CSS class to set avail.
...
def _skaha_parse_price(soup: BeautifulSoup) -> Optional[int]:
    """Extract standard per-person price from rates table (Cost: row)."""
...
def _skaha_parse_description(soup: BeautifulSoup) -> str:
    """Pull intro paragraph(s) as course description."""
...
def _skaha_parse_dates(soup: BeautifulSoup, base_url: str, utm: str) -> list:
    """
    Parse all date entries from a Skaha course page.

    Bookable:  <a class="btn btn_book_now"  href="/bookings/?course=NNN&start_date=YYYY-MM-DD">Book Now</a>
    Sold out:  <a class="btn btn_sold_out"  href="/bookings/?course=NNN&start_date=YYYY-MM-DD" disabled="disabled">Sold Out</a>

    Both carry start_date in the href. We check the CSS class to determine avail.
    """
    today = date.today()
    results = []
    seen = set()
    ...
    for a in soup.find_all("a", href=re.compile(r"/bookings/\?course=\d+&start_date=")):
        ...
        if "btn_sold_out" in classes or a.get("disabled"):
            avail = "sold"
            booking_url = base_url + "/rock-climbing/"  # no bookable URL for sold-out
        else:
            avail = "open"
...
def scrape_skaha() -> list:
    """Scrape all Skaha Rock Adventures course pages."""
    import random
    ...
    for i, course_meta in enumerate(SKAHA_COURSE_PAGES):
        ...
        if not date_entries:
            ...
            all_courses.append({
                ...
                "activity":        course_meta["activity"],
                "activity_raw":    course_meta["activity"],
...
def main():
    ...
    if provider_filter in ("all", "skaha-rock-adventures"):
        ...
        # Batch summaries — deduplicate by title so all dates of same course share one summary
        if processed:
            seen_titles = {}
            unique_inputs = []
            for c in processed:
                if c.get("description") and c["title"] not in seen_titles:
                    ...
                    unique_inputs.append({
                        ...
                        "title":       c["title"],
                        "description": c.get("description", ""),
                        "provider":    SKAHA_PROVIDER["name"],
                        "activity":    c.get("activity_canonical", "climbing"),
                    })
            if unique_inputs:
    ...
    for provider in [p for p in IAG_PROVIDERS if provider_filter in ("all", p["id"])]:
        ...
        processed = []
        for c in raw_courses:
            loc_canonical, loc_is_new, loc_add_mapping = normalise_location(c.get("location_raw",""), mappings)
            if not loc_canonical:
