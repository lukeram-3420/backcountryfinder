#!/usr/bin/env python3
"""scraper_toby_creek_adventures.py — Zaui scraper for Toby Creek Adventures.

Platform: Zaui booking API at https://tobycreekadventures.zaui.net/booking/api
All read endpoints are anonymous — no auth, no CSRF, no Playwright required.

Mirrors scraper_vanmtnguides.py / scraper_mt_norquay.py / scraper_canmore_adventures.py.
Grouped scraper — accepts --group N (0..3). Shared helpers multi-date probe
internally so seasonal categories surface in shoulder seasons.
"""
...
PROVIDER = {
    "id":               "toby-creek-adventures",
    "name":             "Toby Creek Adventures",
    "website":          "http://www.tobycreekadventures.com",
    "tenant_slug":      "tobycreekadventures",
    "portal_id":        1,
    "default_location": "Panorama, BC",
}
...
LOCATION_MAP = [
    ("panorama",     "Panorama, BC"),
    ("invermere",    "Invermere, BC"),
    ("radium",       "Radium Hot Springs, BC"),
    ("kootenay",     "Kootenay, BC"),
    ("purcell",      "Purcell Mountains, BC"),
    ("toby creek",   "Panorama, BC"),
    ("paradise mine","Panorama, BC"),
    ("columbia",     "Invermere, BC"),
]

ZAUI_LOCATION_FIELDS = ("location", "meetingLocation", "address", "venue", "city")

# Non-course products sometimes appear in the Zaui catalogue alongside tours.
EXCLUDE_TITLES = [
    "gift card",
    "gift certificate",
    "deposit",
