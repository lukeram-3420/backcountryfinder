...
async function googlePlacesLookup(name: string, location: string | null) {
  ...
  if (!apiKey) return { google_place_id: null, rating: null, review_count: null };
  ...
  try {
    ...
    if (!r.ok) return { google_place_id: null, rating: null, review_count: null };
    const data = await r.json();
    const candidate = data?.candidates?.[0];
    if (!candidate) return { google_place_id: null, rating: null, review_count: null };
    return {
    ...
    return { google_place_id: null, rating: null, review_count: null };
  }
}
